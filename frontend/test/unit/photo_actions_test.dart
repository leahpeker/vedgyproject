...
void main() {
  ...
  group('PhotoActions', () {
    test(
      'uploadPhotos sends POST with multipart form data and returns photos',
      () async {
        ...
        when(
          ...
          (_) async =>
              okResponse(photoJson, '/api/listings/listing-uuid-001/photos/'),
        );
    ...
    test('uploadPhotos uploads multiple files sequentially', () async {
      ...
      final actions = container.read(photoActionsProvider);
      final result = await actions.uploadPhotos('listing-uuid-001', [
        xfile1,
        xfile2,
      ]);
    ...
    test('deletePhoto sends DELETE to /api/listings/photos/:id/', () async {
      when(
        () => mockDio.delete<void>('/api/listings/photos/photo-uuid-001/'),
      ).thenAnswer(
        (_) async => Response(
          statusCode: 204,
          requestOptions: RequestOptions(
            path: '/api/listings/photos/photo-uuid-001/',
          ),
        ),
      );
