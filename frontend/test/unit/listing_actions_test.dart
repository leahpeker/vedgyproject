...
void main() {
  ...
  group('ListingActions', () {
    test('createListing sends POST to /api/listings/ and returns Listing', () async {
    ...
    test('updateListing sends PATCH to /api/listings/:id/ and returns Listing', () async {
      final fields = {'title': 'Updated Title'};
      when(() => mockDio.patch<Map<String, dynamic>>(
            '/api/listings/listing-uuid-001/',
            data: fields,
          )).thenAnswer((_) async => okResponse(testListingJson, '/api/listings/listing-uuid-001/'));
    ...
    test('deleteListing sends DELETE to /api/listings/:id/', () async {
      when(() => mockDio.delete<void>('/api/listings/listing-uuid-001/'))
          .thenAnswer((_) async => Response(
                statusCode: 204,
                requestOptions: RequestOptions(path: '/api/listings/listing-uuid-001/'),
              ));

      final actions = container.read(listingActionsProvider);
      await actions.deleteListing('listing-uuid-001');

      verify(() => mockDio.delete<void>('/api/listings/listing-uuid-001/')).called(1);
    });

    test('deactivateListing sends POST to /api/listings/:id/deactivate/', () async {
      when(() => mockDio.post<Map<String, dynamic>>(
            '/api/listings/listing-uuid-001/deactivate/',
          )).thenAnswer((_) async => okResponse(
            {...testListingJson, 'status': 'deactivated'},
            '/api/listings/listing-uuid-001/deactivate/',
          ));
    ...
    test('submitForReview sends POST to /api/listings/:id/submit/', () async {
      when(() => mockDio.post<Map<String, dynamic>>(
            ...
            {...testListingJson, 'status': 'payment_submitted'},
            '/api/listings/listing-uuid-001/submit/',
          ));
